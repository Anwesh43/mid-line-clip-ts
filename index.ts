...
class MCLSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new MCLSNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawMCLSNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : MCLSNode {
        var curr : MCLSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class MidClipLineSquare {

    curr : MCLSNode = new MCLSNode(0)
    dir : number = 1
